...
import { ArrowLeft, Save, User, Building2, MapPin, Phone, Mail, Loader2 } from 'lucide-react'
  ...
  useEffect(() => {
    const loadCustomer = async () => {
      ...
      try {
        ...
        setMobile(customer.mobile ? maskPhone(customer.mobile) : '')
        setWebsite(customer.website || '')
        setCreditLimit(customer.creditLimit ? customer.creditLimit.toString() : '')
        setNotes(customer.notes || '')

        // Popular dados específicos Pessoa Física
        if (customer.personType === 'FISICA') {
          setName(customer.name || '')
          setCpf(customer.cpf ? maskCPF(customer.cpf) : '')
          setRg(customer.rg || '')
          setRgIssuer(customer.rgIssuer || '')
          setRgState(customer.rgState || '')
          setBirthDate(customer.birthDate || '')
          setGender(customer.gender || 'MALE')
          setMaritalStatus(customer.maritalStatus || 'SINGLE')
          setMotherName(customer.motherName || '')
          setProfession(customer.profession || '')
          setNationality(customer.nationality || 'Brasileira')
        }

        // Popular dados específicos Pessoa Jurídica
        if (customer.personType === 'JURIDICA') {
          setCompanyName(customer.companyName || '')
          setTradeName(customer.tradeName || '')
          setCnpj(customer.cnpj ? maskCNPJ(customer.cnpj) : '')
          setStateRegistration(customer.stateRegistration || '')
          setStateRegistrationExempt(customer.stateRegistrationExempt || false)
          setCnae(customer.cnae ? maskCNAE(customer.cnae) : '')
          setTaxRegime(customer.taxRegime || 'SIMPLES_NACIONAL')
          setResponsibleName(customer.responsibleName || '')
          setResponsibleCpf(customer.responsibleCpf ? maskCPF(customer.responsibleCpf) : '')
          setResponsibleEmail(customer.responsibleEmail || '')
          setResponsiblePhone(customer.responsiblePhone ? maskPhone(customer.responsiblePhone) : '')
        }

        // Popular endereço
        if (customer.addresses && customer.addresses.length > 0) {
          const mainAddress = customer.addresses.find(addr => addr.type === 'MAIN') || customer.addresses[0]
          setZipCode(mainAddress.zipCode ? maskCEP(mainAddress.zipCode) : '')
          setStreet(mainAddress.street || '')
          setNumber(mainAddress.number || '')
          setComplement(mainAddress.complement || '')
          setNeighborhood(mainAddress.neighborhood || '')
          setCity(mainAddress.city || '')
          setState(mainAddress.state || '')
          setIbgeCode(mainAddress.ibgeCode || '')
        }
      } catch (error: any) {
        toast({
          title: 'Erro',
